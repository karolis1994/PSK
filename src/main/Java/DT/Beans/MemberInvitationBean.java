...
public class MemberInvitationBean{
   ...
   // @Pattern(regexp="[\\w\\.-]*[a-zA-Z0-9_]@[\\w\\.-]*[a-zA-Z0-9]\\.[a-zA-Z][a-zA-Z\\.]*[a-zA-Z]")
    ...
    public void init() {  
        ...
        //allPrincipals = principalsFacade.findAll();
    }
    ...
    public void sendInvitation() throws Exception{
        ...
        if (validate(inputEmail)!= true){
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "", "Jūsų įvestas el. paštas yra neteisingo formato. Bandykite dar kartą"));
            return;
        }
        
        if (principalsFacade.findByEmail(inputEmail) != null){
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "", "Šis vartotojas jau yra sistemoje."));
            return;
        }
        ...
        try {
            ...
            if(error == -1) {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error: ", "Klaida. Laiškas neišsiųstas"));
            } else if(error == 0) {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "", "Laiškas išsiųstas."));
            }
        }
    }
